...
function jRnaInstance (meta, args={}) {
    ...
    this.attach = function( element ) {
        if (!element || !element.length)
            meta.throw( "Cannot attach to a missing element");
        element = element.first();
        this.container = element;
        meta.post_setup(this, element);

        for (let i in this._on_attach ) {
    ...
    this.append_to = function( element ) {
        if (!element || !element.length)
            meta.throw( "Cannot append to a missing element");
        element = element.first();
...
function jRna () {
    ...
    this.upload = function( id, name, type ) {
        ...
        return this.setup( id, function( inst, element ) {
            inst[name] = function() {
                return uploadFile( element[0].files[0], type );
            };
        } );
    };
    ...
    this.attach = function(element, args) {
        const inst = new jRnaInstance(this, args);
    ...
    this.post_setup = function(inst, container) {
        for (let i in this._setup) {
            const action = this._setup[i];
            const all = container.find( "#"+action[0] ).first();
            if (all.length == 0)
                this.throw( "No element #"+action[0]+" inside #"+container.attr("id") );
            action[1](inst, all);
