...
/*
First of all, some macros for jsdoc because it's to boring to write it every time
...
if (typeof window != 'undefined' && typeof window.$ != 'function')
    throw new Error('jRna: refusing to run without a window.$');
...
function jRna () {
    ...
    */
    ...
    const curry = function(item, todo, preargs=[]) {
    ...
    *   @end callback
    */
    this.click = function( id, cb, arg ) {
        return this.setup( id, function( inst, element ) {
            cb = curry( inst, cb, arg );
            element.on( "click", function () { cb(element); return false; } );
        } );
    };
    this.toggle = function( id, cb_on, cb_off ) {
        return this.setup( id, function( inst, element ) {
            cb_on = curry( inst, cb_on );
            cb_off = curry( inst, cb_off );
            let on = false;
    ...
    this.stickyClick = function( id, name, cb, args ) {
        this.lockName( name, 'stickyClick' );
        return this.setup( id, function( inst, element ) {
            cb = curry( inst, cb, args );
            element.on('click', function () {
                if (!inst[name]) {
                    inst[name] = true;
                    cb(element);
    ...
    this.on = function( id, trigger, cb, args ) {
        return this.setup(id, function(inst, element) {
            cb = curry( inst, cb, args );
...
jRna.download = function(filename, content, ctype) {
    ...
    document.body.appendChild(aHref); // required for firefox
    aHref.click();
    aHref.remove();
};
