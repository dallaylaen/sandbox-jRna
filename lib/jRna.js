...
function jRna () {
    ...
    this.html_from = function(selector) {
        selector = this.checkElement(selector);
    ...
    // callbacks!
    this._on_attach = [];
    this.on_attach = function(fun) {
        this._on_attach.push(fun);
        return this;
    };
    ...
    this.checkElement = function(element, action="address") {
        // TODO extract the selector in use & append to error message
        if (!element)
        ...
        if (!element.length)
            this.throw( "Cannot "+action+" a missing element" );
        if ( element.length > 1)
            this.throw( "Cannot "+action+" an ambiguous element" );
    ...
    this.attach = function(container, args) {
        ...
        // create instance
        const inst = new jRnaBound(container);
        ...
        for (let i in meta._setup) {
            ...
            const all = container.find( "."+jRna.prefix+action[0] );
            meta.checkElement(all, 'fulfill .'+jRna.prefix+action[0]+' with');
            action[1](inst, all);
        ...
        // execute callbacks
        // TODO rewrite this
        for (let i in meta._on_attach ) {
            meta._on_attach[i].bind(inst)();
        }
        return inst;
    }; // end of this.attach
